 ...
 * @return {object} Nodemailer transporter configured with Google Workspace SMTP
 */
...
exports.verifyPlayIntegrity = functions.https.onCall(async (data, context) => {
  try {
    ...
    const verifyUrl = `https://playintegrity.googleapis.com/v1/${packageName}:decryptIntegrityToken`;
    ...
    if (payload) {
      ...
      if (payload.deviceIntegrity) {
        console.log(
            "Device Integrity:",
            JSON.stringify(payload.deviceIntegrity.deviceRecognitionVerdict),
        );
      }
